...
const newTemp = async ({name}) => {
    try {
        if(!name) throw 'Falta temperament'
...
const allTemps = async ({temperamento}) => {
    try {
        ...
        if(!resultadoApi) throw 'No es posible obtener datos de la API'
        if(temperamento){
            ...
            if(temps.length === 0) throw `Temperament ${temperamento} no encontrado`
            return temps
        }
